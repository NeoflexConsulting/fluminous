...
import org.fluminous.jq.filter.pattern.dsl.{ InconsistencyMeasure, MatcherInfo }
...
case class ParserState(stack: List[Expression] = List.empty, failInfo: Option[FailInfo] = None) {
  ...
  def saveFailedMatcher(matcherInfo: MatcherInfo, position: Int, i: InconsistencyMeasure): ParserState = {
    this.failInfo

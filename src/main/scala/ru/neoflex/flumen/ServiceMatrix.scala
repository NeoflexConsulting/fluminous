...
object ServiceMatrixCompose {
  def apply[TYPE, TAIL <: ServiceMatrix] (tail : TAIL)(service: Service[TYPE,TYPE],  conditions: Seq[Condition[TYPE]], servicesWitInput : tail.SERVICE_WITH_INPUT_TYPE[TYPE], servicesWithOutput: tail.SERVICES_WITH_OUTPUT_TYPE[TYPE] ) = {

    InnerServiceMatrixCompose(tail,service,conditions, servicesWitInput,servicesWithOutput)
  }

  final case class InnerServiceMatrixCompose [TYPE, TAIL <: ServiceMatrix, TAIL_SERVICES_WITH_INPUT_TYPE<: ServicesWithInput[TYPE],
      TAIL_SERVICES_WITH_OUTPUT_TYPE <: ServicesWithOutput[TYPE]](tail : TAIL,service: Service[TYPE,TYPE], conditions: Seq[Condition[TYPE]], serviceWithInput : TAIL_SERVICES_WITH_INPUT_TYPE, servicesWithOutput: TAIL_SERVICES_WITH_OUTPUT_TYPE ) extends ServiceMatrix {
    ...
    def enlarge[A](servicesColumn : this.SERVICE_WITH_INPUT_TYPE[A], servicesRow: this.SERVICES_WITH_OUTPUT_TYPE[A], service: Service[A,A], conditions: Seq[Condition[A]] = Seq.empty[Condition[A]]) = {
      ServiceMatrixCompose(this)(service,conditions,servicesColumn,servicesRow)
    }
    def get[I,O](implicit get: MatrixGet[THIS,I,O]):Service[I,O] = get(this)
...
object ServiceMatrix {
  ...
  def apply[W](service: Service[W,W]) = {
    ServiceMatrixCompose(NilServiceMatrix)(service,Seq.empty[Condition[W]], ServicesWithInputNil[W], ServicesWithOutputNil[W])
  }
}
